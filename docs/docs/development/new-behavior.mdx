...
Behaviors refer to the actions that are invoked when a key is pressed or released.
...
:::info
If a behavior's use-case is deemed widespread enough to be implemented in upstream ZMK, the new behavior may be submited as a pull request.
Instead of a separate module repository, the procedures described below would be applied to a fork of ZMK.
Although we describe practices that maximize compatibility between module-focused development and upstream workflows, slight modification may be necessary when creating the pull request.
:::

In the context of the Zephyr RTOS, behaviors are implemented as "devices", which contain:

- A devicetree binding file, which declare behavior's properties
- A device driver written in C code
- Optionally, devicetree-source-include files (`.dtsi`), which contain predefined instances of the behavior that

The general process for developing behaviors is:

1. [Create a new behavior repository](#creating-a-new-behavior-module)
   1. [Initialize a new ZMK module for the behavior](#initializing-a-new-behavior-module)
   1. [Prepare the module with the appropriate filesystem](#preparing-the-filesystem)
1. [Create the behavior](#creating-the-behavior)
   ...
   1. [Update `CMakeLists.txt` to include the new driver during compilation](#updating-cmakeliststxt)
...
1. Clone a copy of your module to your development environment.
   It is easiest if your behavior resides in the same folder as your copy of the ZMK source code, as shown in the tree below.
...
Broadly speaking, they mostly appear as `.gitkeep` files found in unnecessary subfolders.
The minimum viable filesystem for a behavior follows the following structure:

```
zmk-behavior-hello-world/
├── CMakeLists.txt
├── Kconfig
├── LICENSE
├── README.md
...
│   ├── behaviors
// highlight-next-line
│   │   └── hello_world.dtsi (optional)
│   └── bindings
│       └── behaviors
// highlight-next-line
│           └── zmk,behavior-hello-world.yaml
├── include
│   └── dt-bindings
│       └── zmk
// highlight-next-line
│           └── hello_world.h (optional)
├── src
│   └── behaviors
// highlight-next-line
│       └── behavior_hello_world.c
// highlight-start
├── tests
│   └── hello_world
...
For more information on module preparation, refer to the page on [module creation](./module-creation.md).
This is especially the case for procedures involving the `west.yml` and `zephyr/module.yml` files.
...
For example, the directory for the hold-tap's devicetree binding would be located at `app/dts/bindings/behaviors/zmk,behavior-hold-tap.yaml`, which is shown below as a reference:
...
Since the value of this property is not seen by end-users, the `description` should be kept less than a sentence long.
Detailed explanations for end-users of how the behavior works can be shared in the behavior's README.

#### `compatible`

Allows ZMK to assign the correct driver to the behavior extracted from the keymap or `.dtsi`.
The value of the `compatible` property is same as the name of the [devicetree binding file](#creating-the-devicetree-binding-yaml).
...
For example, we `include: two_param.yaml` in `zmk,behavior-hold-tap.yaml` because any user-defined or pre-defined instances of the hold-tap behavior take in two cells as inputs: one for the hold behavior and one for the tap behavior.
...
Driver files are stored in `app/src/behaviors/` and are labelled in lowercase, beginning with the prefix `behavior_`, and ending with the behavior's name, using underscores to separate multiple words.
For example, the directory for the hold-tap's driver would be located at `app/src/behaviors/behavior_hold_tap.c`.
...
Events can be used similarly to hardware interrupts.
See [Events](events.md) for more information on using events.
...
`BEHAVIOR_DT_INST_DEFINE` is a special ZMK macro.
It forwards all the parameters to Zephyr's `DEVICE_DT_INST_DEFINE` macro to define the driver instance, then it adds the driver to a list of ZMK behaviors so they can be found by `zmk_behavior_get_binding()`.
...
Comes in the form `static int <behavior_name>_init(const struct device *dev)`.
...
Comes in the form `static const struct behavior_driver_api <behavior_name>_driver_api)`.
Common items to include in the API Structure are:

- `.binding_pressed`: Used for behaviors that invoke an action on its keybind press.
  Set `.binding_pressed` equal to the function typically named [`on_<behavior_name>_binding_pressed`](#dependencies).
- `.binding_released`: Same as above, except for activating on keybind release events.
  Set `.binding_released` equal to the function typically named [`on_<behavior_name>_binding_released`](#dependencies).
- `.parameter_metadata`: Defined in `<drivers/behavior.h>`.
  Pointer to metadata describing the parameters to use with the behavior so the behavior may be used with [ZMK Studio](../features/studio.md).
- `.get_parameter_metadata`: Defined in `<drivers/behavior.h>`.
  Callback function that can dynamically provide/populate the metadata describing the parameters to use with the behavior so the behavior may be used with [ZMK Studio](../features/studio.md).
- `.locality`: Defined in `<drivers/behavior.h>`.
  Describes how the behavior affects parts of a _split_ keyboard.
...
You can see this with the `&bt` behavior.
...
Regardless of the instance number, `n`, `behavior_<behavior_name>_data_##n` is typically initialized as an empty `struct`.
The data respective to each instance of the behavior can be accessed in functions like [`on_<behavior_name>_binding_pressed(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event)`](#dependencies) by extracting the behavior device from the keybind like so:
...
As seen in the `#define KP_INST(n)` of the hold-tap example, the configuration `struct`, `behavior_<behavior_name>_config_##n`, for each instance number, `n`, can be initialized using the [Zephyr Devicetree Instance-based APIs](https://docs.zephyrproject.org/3.5.0/build/dts/api/api.html#instance-based-apis), which extract the values from the `properties` of each instance of the [devicetree binding](#creating-the-devicetree-binding-yaml) from a user's keymap or [predefined use-case `.dtsi` files](#defining-common-use-cases-for-the-behavior-dtsi-optional) stored in `app/dts/behaviors/`.
We illustrate this further by comparing the [`#define KP_INST(n)` from the hold-tap driver](#behavior_dt_inst_define) and the [`properties` of the hold-tap devicetree binding](#creating-the-devicetree-binding-yaml).
...
Most behavior drivers' are invoked according to the central half's [locality](#api-structure), and are therefore stored after the line `if ((NOT CONFIG_ZMK_SPLIT) OR CONFIG_ZMK_SPLIT_ROLE_CENTRAL)`.
in the form, `target_sources(app PRIVATE src/behaviors/<behavior_name>.c)`, as shown below.
...
if ((NOT CONFIG_ZMK_SPLIT) OR CONFIG_ZMK_SPLIT_ROLE_CENTRAL)
	target_sources(app PRIVATE src/behaviors/behavior_hello_world.c)
endif() # ((NOT CONFIG_ZMK_SPLIT) OR CONFIG_ZMK_SPLIT_ROLE_CENTRAL)
```

For behaviors that do not require [central locality](../features/split-keyboards.md#behaviors-with-locality), the following options for updating `CMakeLists.txt` also exist:
...
:::info
If submitting a pull request to upstream ZMK, the `target_sources` invocation would go inside `app/CMakeLists.txt` instead.
:::
...
A common example is the mod-tap (`&mt`), which is a predefined type of hold-tap that takes a modifier key as the hold parameter and another key as the tap parameter.
...
The only major difference is that the value `/omit-if-no-ref/` should be placed adjacent to the label and name of the behavior, as seen in line 11 of the `gresc` example.
...
After creating the `.dtsi` from above, you may include the pre-defined use-case in your keymap by adding
`#include <behaviors/new_behavior_instance.dtsi>` at the top of the keymap.

:::info
If submitting a pull request to upstream ZMK, this `#include` statement would go inside `app/dts/behaviors.dtsi`.
As most keymaps `#include <behaviors.dtsi>`, this eliminates the need to include your new behavior's specific `.dtsi`
:::

## Testing Changes Locally

Create a new folder in `app/tests/` to develop virtual test sets for all common use cases of the behavior.
Behaviors should be tested thoroughly on both virtual testing environments using `west test` and real hardware.
...
Once the functional portions of each file are completed, their copyright information must be updated next.
The appropriate copyright headers for the files which can be copied and pasted from the tabs below.
